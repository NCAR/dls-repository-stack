...
/**
 *  A factory container for a hit that is returned from a {@link edu.ucar.dls.index.SimpleLuceneIndex}
 *  search. This factory uses the class name provided in the {@link
 *  edu.ucar.dls.index.writer.DocWriter#getReaderClass()} method to return the {@link
 *  edu.ucar.dls.index.reader.DocReader} bean that is best suited to read the type of index {@link
 *  org.apache.lucene.document.Document} that was returned by the search. Also provides access to a {@link
 *  edu.ucar.dls.index.reader.DocumentMap} for reading the fields in the lucene Document. This container also
...
public class ResultDoc implements Comparable, Serializable {
	...
	/**
	 *  Gets the {@link edu.ucar.dls.index.reader.DocReader} used to read the specific {@link
	...
	public final DocReader getDocReader() {
		...
		try {
			...
			try {
                ...
                // Convert legacy org.dlese.dpc package names:
				if(className.startsWith("org.dlese.dpc"))
                    className = className.replace("org.dlese.dpc","edu.ucar.dls");
	...
	/**
	 *  Constructor for the ResultDoc that is used by the {@link edu.ucar.dls.index.SimpleLuceneIndex} class
