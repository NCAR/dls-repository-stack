...
public final class WebLogIndexingService {
	...
	public long indexWebLogFile(File webLogFile)
		...
		DocumentWrapper newDocWrapper = null;
		try{
			String logEntry = "";
			...
			while (logEntry != null) {
				...
				if(logEntry != null && logEntry.trim().length() > 0) {
					...
					newDocWrapper = writer.createLogEntryDocWrapper(logEntry);
					if(newDocWrapper != null){
						indexDocs.add(newDocWrapper);
						totalIndexed++;
						i++;
					}
				}
				...
				if(i == MAX_ITEMS){
					...
					try {
						webLogIndex.addDocs((DocumentWrapper[])indexDocs.toArray(new DocumentWrapper[]{}));
