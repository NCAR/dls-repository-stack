 ...
 *  {@link edu.ucar.dls.index.FileIndexingService} to enable automatic updating of the index whenever changes
 *  in the source file are made. This class, along with the {@link edu.ucar.dls.index.FileIndexingService},
 *  may be used with a {@link edu.ucar.dls.index.SimpleLuceneIndex} to provide simple search support over
 *  files.<p>
 *
 *  Note: after creating a new concrete FileIndexingServiceWriter, add a switch in {@link
 *  edu.ucar.dls.repository.RepositoryManager}, method putDirInIndex(DirInfo, String) to select it for
 *  indexing.<p>
 *
 *  <br>
 ...
 *    edu.ucar.dls.index.FileIndexingService} for updating/deleting and may be used by beans or other classes
 ...
 *    the {@link edu.ucar.dls.index.FileIndexingService} for updating/deleting and may be used by beans or
 *    other classes.</li>
 *    <li> <code>modtime</code> - The file modification time, which is used by the {@link
 *    edu.ucar.dls.index.FileIndexingService} to determine if the file has changed and needs update and may
...
public abstract class FileIndexingServiceWriter implements DocWriter {
	...
	/**
	 *  Gets the fully qualified name of the concrete {@link edu.ucar.dls.index.reader.DocReader} class that is
	 *  used to read this type of {@link org.apache.lucene.document.Document}, for example
	 *  "edu.ucar.dls.index.reader.ItemDocReader".
	 *
	 ...
	 *  {@link edu.ucar.dls.index.FileIndexingService#addDirectory} method.
	 *
	 ...
	 *  <code>}</code>
	 *
	 * @param  newDocumentWrapper             The new DocumentWrapper that is being created for
	 *      this resource
	 ...
	 *      error occurs.
	 */
	protected abstract void addCustomFields(DocumentWrapper newDocumentWrapper, Document previousRecordDoc,  Document previousIndexExistingLuceneDoc, File sourceFile) throws Exception;
	...
	/**
	 *  Gets the Lucene DocumentWrapper that this Writer is building.
	 *
	 * @return    The Lucene DocumentWrapper
	 */
	...
	/**
	 *  Sets the Lucene DocumentWrapper that this Writer is building. Normally, this would be created internally, but
	 *  this method may be used to set a specific DocumentWrapper to work with such as the case when indexing a
	 *  multirecord.
	 *
	 * @param  luceneDoc  The new documentWrapper value
	 */
	public void setDocumentWrapper(DocumentWrapper documentWrapper) {
	 ...
	 *  This value is set by the {@link edu.ucar.dls.index.FileIndexingService} prior to indexing. If true, the
	 ...
	 * @see                   edu.ucar.dls.index.FileIndexingService#setValidationEnabled(boolean validateFiles)
	 */
	...
	protected void addToDefaultField(String value) {
		if (value != null && value.trim().length() > 0)
			IndexingTools.addToDefaultAndStemsFields(getDocumentWrapper().getDocument(), value);
	}
	...
	protected void addToAdminDefaultField(String value) {
		if (value != null && value.trim().length() > 0)
			IndexingTools.addToAdminDefaultField(getDocumentWrapper().getDocument(), value);
	}
	 ...
	 * @param  existingLuceneDoc               An existing Document that resides in the working index for the
	 *      given resource, or null if none was present
	 * @param  previousIndexExistingLuceneDoc  An existing Document that resides in the previous index for the
	 *      given resource, or null if none present. Used when background indexing is being used to preserve proper OAI mod time.
	...
	public synchronized FileIndexingServiceData create(File sourceFile,
	                                                   ...
	                                                   Document previousIndexExistingLuceneDoc,
		 ...
		 */
			...
			addCustomFields(getDocumentWrapper(), previousRecordDoc, previousIndexExistingLuceneDoc, source);
			...
			newDocData.setDocWrapper(getDocumentWrapper());
