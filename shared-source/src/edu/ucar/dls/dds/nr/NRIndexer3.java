 ...
 */
public class NRIndexer3 implements ItemIndexer {
    private static boolean debug = true;

    // block size or records to index on, at a time
    private int INDEX_BLOCK_SIZE = 500;
    ...
    // Should collections be allowed to submit more than one (duplicate) record per resource URL (set to false to filter out)?
    ...
    private boolean allowDuplicatURLsInCollections = true;
    ...
    private void configureAndInitialize() throws Exception {
        synchronized (this) {
            ...
            // Allow duplicate records in collections (otherwise skip those records)?:
            String allowDuplicatURLsInCollectionsStr = configXmlDoc.valueOf("/NR_Collections/allowDuplicatURLsInCollections");
            if (allowDuplicatURLsInCollectionsStr != null && allowDuplicatURLsInCollectionsStr.equalsIgnoreCase("false"))
                allowDuplicatURLsInCollections = false;
    ...
    public void indexCollections(String collectionKey) throws Exception {
        ...
        try {
            ...
            // Call custom sort to to put comm_anno and comm_para first, then lar, then nsdl_dc
            // This creates the lease amount of relationship dependencies to cut down on the
            // amount of re-indexing.
            Collections.sort(nrCollectionInfos, EfficientIndexingCollectionInfoComparator);
     ...
     * @throws Exception If error
     */
    private void doIndexItemsInCollection(CollectionInfo collectionInfo) throws Exception {
        if (abortIndexing)
            return;
        String collectionKey = collectionInfo.getCollectionKey();
        String formatToIndex = collectionInfo.getFormatToIndex();
        ...
        HashMap<String,Object> urlsFoundInCollection = new HashMap<String, Object>();
        ...
        PreparedStatement pstResourceData = null;
        ...
        try {
            ...
            // Select the resource handle and URL for each metadata record (statement is reused for each metadata record):
            pstResourceData = con.prepareStatement("SELECT resourcehandle,URL FROM resource WHERE metadatahandle=? AND sessionid='" + sessionid + "'");
            List<HashMap<String, String>> recordsToIndex = new ArrayList<HashMap<String, String>>();
            int count = 0;
            while (numRecords > 0 && metadataRecordResults.next() && !abortIndexing && numProcessed < MAX_RECORDS_PER_COLLECTION) {
                ...
                try {
                    ...
                    // This seems like unnecessary logging that was slowing machine down, commenting out
                    Date startDate = new Date();
                    prtln("Processing record start " + metadataHandle + " format: " + formatToIndex);

                    boolean skipProcessingRecord = false;
                    String skipProcessingReasonMsg = "";
                    ...
                    if (formatToIndex.equals("nsdl_dc") || formatToIndex.equals("lar")) {
                        ...
                        pstResourceData.setString(1, metadataHandle);
                        resourceHandleResults = pstResourceData.executeQuery();
                        while (resourceHandleResults.next()) {
                            resourceHandles.add(resourceHandleResults.getString("resourcehandle"));

                            if(!allowDuplicatURLsInCollections) {
                                String myURL = resourceHandleResults.getString("URL");
                                if (urlsFoundInCollection.containsKey(myURL)) {
                                    skipProcessingRecord = true;
                                    skipProcessingReasonMsg += " Duplicate URL in collection " + collectionName + " '" + myURL + "'";
                                }
                                else
                                    urlsFoundInCollection.put(myURL,null);
                            }
                        }
                        resourceHandleResults.close();
                        if (resourceHandles.size() == 0) {
                            numMissingResourceHandle++;
                            printStatusMessageWarning("metadataHandle '" + metadataHandle + "' has 0 resource handles (using query='" + pstResourceData + "')");
                        }
                    }
                    partnerId = metadataRecordResults.getString("partnerid");

                    // Special filter for NASA Wavelength to clean up records that should not be there (bug in their OAI feed):
                    if(collectionKey.startsWith("ncs-NSDL-COLLECTION-000-003-112-115") && partnerId.startsWith("retired-")){
                        skipProcessingRecord = true;
                        skipProcessingReasonMsg += " NASA Wavelength retired record.";
                    }
                    ...
                    if (!skipProcessingRecord) {
                        ...
                        recordMap.put("recordXml", Dom4jUtils.prettyPrint(itemRecord));
                        recordMap.put("id", ddsId);
                        recordsToIndex.add(recordMap);
                        numProcessed++;
                        ...
                        prtln("Processing record end " + metadataHandle + " num processed: " + numProcessed + " processing time" + alertMsg + ": " + elapsedTime + " ms");

                        if (recordsToIndex.size() >= INDEX_BLOCK_SIZE) {
                            int numToIndex = recordsToIndex.size();
                            try {
                                collectionIndexer.putRecords(recordsToIndex, collectionKey, collectionIndexingSession);
                                String msg = String.format("Re-indexed record block %d-%d for collection %s successfully", numProcessed - numToIndex + 1, numProcessed, collectionName);
                                printStatusMessage(msg);
                                numSuccess += INDEX_BLOCK_SIZE;
                            } catch (Exception e) {
                                String msg = String.format("Record block %d-%d could not be indexed for collection %s. Reason %s", numProcessed - numToIndex + 1, numProcessed, collectionName, e);
                                printStatusMessageError(msg);
                                numErrors += numToIndex;
                            }
                            recordsToIndex.clear();
                        }
                    }
                    // If skipped for some reason (not done, etc.):
                    else {
                        String msg = "Record " + metadataHandle + " was skipped for non-critical reason." + skipProcessingReasonMsg;
                        prtln(msg);
                        numSkippedNonFatal++;
                        numProcessed++;
                    }
                    ...
                    printStatusMessageError("could not get itemRecord for " + metadataHandle + ": " + e);
                    numErrors++;
                    numProcessed++;
                } finally {
            ...
            if (recordsToIndex.size() > 0) {
                ...
                try {
                    collectionIndexer.putRecords(recordsToIndex, collectionKey, collectionIndexingSession);
                    String msg = String.format("Indexed record block %d-%d for collection %s successfully", numProcessed - numToIndex + 1, numProcessed, collectionName);
                    printStatusMessage(msg);
                    numSuccess += numToIndex;
                } catch (Exception e) {
                    String msg = String.format("Record block %d-%d could not be indexed for collection %s. Reason %s", numProcessed - numToIndex + 1, numProcessed, collectionName, e);
                    printStatusMessageError(msg);
                    numErrors += numToIndex;
            ...
            try {
                ...
                if (pstResourceData != null) {
                    pstResourceData.close();
                }
                if (con != null) {
                    con.close();
                }
    ...
    private void printStatusMessage(String msg) {
        collectionIndexer.printStatusMessage("NRIndexer3: " + msg, CollectionIndexer.MSG_TYPE_INFO);
        prtln(msg);
    }

    private void printStatusMessageWarning(String msg) {
        collectionIndexer.printStatusMessage("NRIndexer3: " + msg, CollectionIndexer.MSG_TYPE_WARNING);
        prtlnWarning(msg);
    }

    private void printStatusMessageError(String msg) {
        collectionIndexer.printStatusMessage("NRIndexer3: " + msg, CollectionIndexer.MSG_TYPE_ERROR);
        prtlnErr(msg);
    }
    ...
    public static Comparator<CollectionInfo> EfficientIndexingCollectionInfoComparator
            = new Comparator<CollectionInfo>() {
        public int compare(CollectionInfo collection1, CollectionInfo collection2) {
            String collection1format = collection1.getFormatToIndex();
            String collection2format = collection2.getFormatToIndex();


            int orderCollection1 = 1;
            if (collection1format.equals("nsdl_dc"))
                orderCollection1 = 3;
            else if (collection1format.equals("lar"))
                orderCollection1 = 2;

            int orderCollection2 = 1;
            if (collection2format.equals("nsdl_dc"))
                orderCollection2 = 3;
            else if (collection2format.equals("lar"))
                orderCollection2 = 2;
            return new Integer(orderCollection1).compareTo(new Integer(orderCollection2));
    ...
    private final static void prtlnErr(String s) {
        System.err.println(getDateStamp() + " NRIndexer3 Error: " + s);
    }
    ...
    private final static void prtlnWarning(String s) {
        System.err.println(getDateStamp() + " NRIndexer3 Warning: " + s);
    }
    ...
    private final static void prtln(String s) {
        if (debug) {
            System.out.println(getDateStamp() + " NRIndexer3: " + s);
        }
    }
