...
/** DEPRECATED, use NRIndexer2
 *  Indexes and configures collections from the NSDL Repository (NR) (MySQL implementation).
 *  8/8/2013
 *  This is the old version of the NRIndexer which included normalization as well as indexing.
 *  This normalization was removed and placed in the harvest manager so that all normalization
 *  and error handling are taken care of there. 
 * @author    John Weatherley
 */
public class NRIndexer implements ItemIndexer {
	...
	private void configureAndInitialize() throws Exception {
		synchronized (this) {
			...
			nsdlDcVocabSelections = configXmlDoc.valueOf("/NR_Collections/nsdlDcVocabSelections");
			writeVerboseComments = configXmlDoc.valueOf("/NR_Collections/writeVerboseComments");
		}
	}
			...
			while (numRecords > 0 && metadataRecordResults.next() && !abortIndexing && numProcessed < MAX_RECORDS_PER_COLLECTION) {
				...
				try {
					...
					if (!skipProcessingRecord) {
						...
						if (formatToIndex != "nsdl_dc")
							contentBlob = metadataRecordResults.getBlob("native_xml");
						else
							contentBlob = metadataRecordResults.getBlob("target_xml");
						...
						// Other formats:
							...
							if (formatToIndex.equals("lar")) {
								...
								if (recordIDNode == null) {
									recordIDNode = itemRecord.getRootElement().addElement("recordID"); // Places the element at the bottom...

									// The 'recordId' element *must* go at the top of the record to be valid:
									recordIDNode.detach();
									List recordElements = itemRecord.getRootElement().elements();
									recordElements.add(0, recordIDNode);
								}
								if (recordIDNode != null) {
									...
									if (isRecastAsLarRelatedMetadataCollection)
										newId = metadataHandle + "-LAR";
									else
										newId = metadataHandle;
									
									//prtln("Setting LAR recordID '" + originalRecordID + "' to '" + newId + "'");
								...
								// Attach the various identifiers (these *must* go at the bottom of the record in this order):
								if (partnerId != null)
					...
					//msg += ". XML record is:\n" + Dom4jUtils.prettyPrint(itemRecord);
