...
import edu.ucar.dls.schemedit.RecordList;
import edu.ucar.dls.schemedit.SchemEditUtils;
import edu.ucar.dls.schemedit.url.DupSimUrlChecker;
...
import edu.ucar.dls.schemedit.repository.RepositoryWriter;
...
/**
 *  Methods to wrap the {@link edu.ucar.dls.repository.RepositoryManager} class
...
public class RepositoryService {
	...
	private Map collectionsOfCollectionRecords = null;
	...
	/**
	 *  Builds a mapping of setSpec to handleService baseUrl from initParameters.
	 *  The records contained in the collections associated with the configured
	 *  setSpecs will be assigned metadataHandles and treated as collection-level
	 *  metadata.
	 *
	 * @return    The collectionOfCollectionRecords value
	 */
	public Map getCollectionOfCollectionRecords() {
		if (this.collectionsOfCollectionRecords == null) {
			prtln("computing collectionsOfCollectionRecords");
			this.collectionsOfCollectionRecords = new HashMap();
			Enumeration enumeration = servletContext.getInitParameterNames();
			String param = null;
			while (enumeration.hasMoreElements()) {
				param = (String) enumeration.nextElement();
				if (param.toLowerCase().startsWith("collectionofcollectionrecords")) {
					String paramVal = servletContext.getInitParameter(param);
					String[] vals = paramVal.split("\\|");
					if (vals.length != 2) {
						prtlnErr("getCollectionOfCollectionRecords() error: could not parse parameter '" + param + "'");
						continue;
					}
					this.collectionsOfCollectionRecords.put(vals[0], vals[1]);
					prtln("  " + vals[0] + " --> " + vals[1]);
				}
			}
		}
		return this.collectionsOfCollectionRecords;
	}


	/**
	 *  Tests whether provided setSpec is configured as a collection of
	 *  collectionRecords, i.e., metadata records that contain collection-level
	 *  information.
	 *
	 * @param  setSpec  the setSpec (aka collectionKey) to test
	 * @return          true if provided setSpec is configured as collectionOfCollections
	 *      Record
	 */
	public boolean isCollectionOfCollectionRecords(String setSpec) {
		return this.getCollectionOfCollectionRecords().containsKey(setSpec);
	}


	/**
	 *  Gets the handleServiceBaseUrl attribute of the RepositoryService object
	 *
	 * @param  setSpec  a setSpec
	 * @return          The configured handleServiceBaseUrl for the setSpec or null
	 *      if setSpec has not been configured.
	 */
	public String getHandleServiceBaseUrl(String setSpec) {
		return (String) this.getCollectionOfCollectionRecords().get(setSpec);
	}
	 ...
	 *
	 *  The metadata Documentn is validated and the dcsDataRecord updated with
	 *  validation and lastTouch and lastEditor information (the status note and
	 ...
	 * @exception  Exception  if record is not found in index or if unable to
	 *      successfully update index
	 */
	 ...
	 * @param  srcRecordId     NOT YET DOCUMENTED
	 * @return                 NOT YET DOCUMENTED
	 * @exception  Exception   NOT YET DOCUMENTED
	 */
	...
	*/
	 ...
	 * @param  eventData  NOT YET DOCUMENTED
	 */
