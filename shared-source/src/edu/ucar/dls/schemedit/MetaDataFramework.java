...
import edu.ucar.dls.schemedit.input.EnsureMinimalDocument;
import edu.ucar.dls.schemedit.input.ElementsOrderer;
import edu.ucar.dls.schemedit.display.*;
import edu.ucar.dls.schemedit.vocab.FieldInfoReader;
import edu.ucar.dls.schemedit.vocab.FieldInfoMap;
import edu.ucar.dls.schemedit.vocab.layout.VocabLayoutConfig;
import edu.ucar.dls.schemedit.vocab.layout.VocabLayout;
 ...
 *  file, and registered in a {@link edu.ucar.dls.schemedit.FrameworkRegistry}.
 *  <p>
 *
 *  Major components include:
 *  <ul>
 *    <li> a {@link edu.ucar.dls.xml.schema.SchemaHelper} instance</li>
 *    <li> a {@link edu.ucar.dls.schemedit.vocab.FieldInfoMap} for information
 *    such as best practices and definitions</li>
 *    <li> a {@link edu.ucar.dls.schemedit.config.SchemaPathMap}</li>
 *    <li> a {@link edu.ucar.dls.schemedit.config.FrameworkConfigReader} that
...
public class MetaDataFramework implements Serializable {
	...
	/**
	* returns true if provided path is 'virtual' in the sense that it is
	* used in editor but is not defined in schema. For example, the LAR editor
	* uses two paths for ASN standards that are not defined in the LAR schema.
	*/
	public static boolean isVirtualPath (String path, String xmlFormat) {
		return (xmlFormat.equals("lar") &&
				(path.equals ("/record/educational/ASN_standard_field") ||
		         path.equals ("/record/educational/ASN_standards_selector")));
	}
		...
		if (this.schemaHelper != null && fieldInfoMap != null) {
			for (Iterator i = fieldInfoMap.getAllFieldInfo().iterator(); i.hasNext(); ) {
				...
				if (this.schemaHelper.getSchemaNode(path) == null &&
					!this.isVirtualPath(path, this.getXmlFormat())) {
	...
	/**
	 *  Update config file with current values for this framework.
	 *
	...
	/**
	 *  Gets the pageList attribute of the MetaDataFramework object, which
	 *  specifies a structure for the metadata editor in which metadata fields are
	 *  associated with individual pages.
	 *
	 * @return    The pageList value
	 */
	public PageList getPageList() {
		// prtln ("getPageList() --> " + this.getXmlFormat());
		VirtualPageConfig vpConfig = VirtualPageConfig.getInstance();
		if (vpConfig != null) {
			VirtualPageList formatConfig = vpConfig.getConfig(this.getXmlFormat());
			if (formatConfig != null) {
				// prtln ("returning VIRTUAL Page List");
				return formatConfig;
			}
		}
	...
	// must be xpath with at least two segments
	Pattern virtualPageFieldPattern = Pattern.compile("(\\/[^\\/]+\\/[^\\/]+).*");

	public boolean hasVirtualPageList () {
		PageList pageList = this.getPageList();
		return (pageList instanceof VirtualPageList);
	}
	
	/**
	 *  Returns name of the page in the editor associated with the given xpath
	 *  (representing a metadata field). The page a particular field is on depends
	 *  on how the instance documents for a particular framework are structured in
	 *  the editor. There are three possibilities:
	 *  <ul>
	 *    <li> No structuring - all fields will be found on the same page</li>
	 *    <li> Schema-based structuring - each top-level element becomes its own
	 *    page. The pages are represented by a {@link PageList} object</li>
	 *    <li> Virtual structuring - the pages are represented by a {@link
	 *    VirtualPageList} object.
	 *  </ul>
	 *
	 *
	 * @param  xpath  represents a metadata editor field
	 * @return        The page value
	 */
	public String getPage(String xpath) {
		PageList pageList = this.getPageList();
		if (pageList == null) {
			return this.getRootElementName();
		}
		else if (pageList instanceof VirtualPageList) {
			VirtualPageList vpl = (VirtualPageList) pageList;
			// ensure there is at least
			String decoded = XPathUtils.decodeXPath(xpath);
			String normalizedXpath = XPathUtils.normalizeXPath(decoded);
			Matcher matcher = virtualPageFieldPattern.matcher(normalizedXpath);
			if (matcher.matches())
				return vpl.getPageName(matcher.group(1));
			else {
				prtlnErr ("could not get virtual page for " + xpath); 
				return null;
			}
		}
		else {
			// pages are defined by a PageList instance
			try {
				return xpath.split("/")[this.getBaseRenderLevel() - 1];
			} catch (Throwable e) {
				prtln("could not get page for xpath: " + xpath + ", xmlFormat: " + this.getXmlFormat());
			}
		}
		return null;
	}
		...
		for (Iterator i = schemaPaths.iterator(); i.hasNext(); ) {
			...
			*/
			...
			if (pathName.equals("id")) {
				...
				value = SchemEditUtils.getUtcDateFormat().format(new Date());
			}
