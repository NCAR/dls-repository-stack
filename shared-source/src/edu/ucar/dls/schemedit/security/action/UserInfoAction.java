...
import edu.ucar.dls.schemedit.security.action.form.UserInfoForm;
				...
				if (userIsAdmin != null) {
 				...
 				if (userForm.getFileLoginEnabled()) {
					...
					if (userForm.getPasswordOptional() && 
						...
						try {
							PasswordHelper.getInstance().update(username, Encrypter.encryptBaseSHA164(userForm.getPass()));
						} catch (Exception e) {
							e.printStackTrace();
