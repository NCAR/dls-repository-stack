...
import edu.ucar.dls.schemedit.input.*;
import edu.ucar.dls.schemedit.autoform.RendererHelper;
...
import edu.ucar.dls.schemedit.standards.asn.AsnWithAttributesServiceHelper;
...
public class SchemEditForm extends ActionForm {
	...
	public String getCurrentPageEncoded() {
		...
		prtln ("getCurrentPageEncoded (currentPage is \"" + currentPage + "\")");
		try {
	...
	/**
	 *  Returns a {@link edu.ucar.dls.schemedit.vocab.FieldInfoReader} for the
...
*/
	...
	public String getViewNode(String key) {
		...
		if (schemaNode != null) {
			if (schemaNode.isAttribute()) {
				...
				requiredContent = schemaHelper.isRequiredContentElement(schemaNode, this.getXmlFormat());
			}
		}
		else {
	...
	public String getNodeIsExpandable(String key) {
		...
		// prtln ("getNodeIsExpandable() xpath: " + xpath);
		...
		// ensure that compositor nodes are expanded (so children, including choice, are always visible)
		SchemaNode schemaNode = this.schemaHelper.getSchemaNode (xpath);
		if (docMap.nodeExists(xpath) && schemaNode.hasCompositor()) {
			return TRUE;
		}
	...
	public LabelValueBean[] getEnumerationOptions(String encodedPath) {
		...
		- BUT we don't want to do this if we are dealing with a comboUnion, since all values are permitted */
		boolean isComboUnion = false;
		String xpath = XPathUtils.decodeXPath(encodedPath);
		try {
			SchemaNode schemaNode = schemaHelper.getSchemaNode(xpath);
			isComboUnion = schemaHelper.isComboUnionType(schemaNode.getValidatingType());
		} catch (Throwable t) {
			prtlnErr ("could not determine if field at " + xpath + " is comboUnion: " + t);
		}
		
		String currentValue = (String)this.docMap.get(xpath);
 		if (!isComboUnion && currentValue != null && currentValue.trim().length() > 0
	...
	public LabelValueBean[] getSubstitutionGroupOptions(String encodedPath) {
		...
		for (int i = 0; i < labels.size(); i++) {
			...
			// index the values if appropriate
			int index = this.getMemberCountOf(value);
			if (index > 0)
				value = value + "_" + index + "_";
	 ...
	 *  edu.ucar.dls.schemedit.input.SchemEditValidator#pruneRepeatingFields}
	 */
		...
		try {
			...
			*/
			if (getFramework() != null) {
				...
				CATServiceHelper standardsServiceHelper = this.getSuggestionServiceHelper();
				
				/* some standards serviceHelpers require preprocessing of the request to update
				   the docMap before passing control to the Action ... */
				if (standardsServiceHelper != null) {
				
					// res_qual hack - update docMap with selected standards
					if (standardsServiceHelper instanceof ResQualSuggestionServiceHelper) {
						try {
							((ResQualSuggestionServiceHelper) standardsServiceHelper).rectifyInstanceDoc(request);
						} catch (Throwable t) {
							prtlnErr("resQual instanceDoc Error: " + t.getMessage());
						}
					}
					
					// lar hack - update docMap with selected standards
					if (standardsServiceHelper instanceof LarSuggestionServiceHelper) {
						try {
							((LarSuggestionServiceHelper) standardsServiceHelper).rectifyInstanceDoc(request);
						} catch (Throwable t) {
							prtlnErr("Lar instanceDoc Error: " + t.getMessage());
							t.printStackTrace();
						}
					}
					
					// frameworks that attach attributes to asnStandard node (incl. "comm_anno" and "cow_item)
					if (standardsServiceHelper instanceof AsnWithAttributesServiceHelper) {
						try {
							((AsnWithAttributesServiceHelper) standardsServiceHelper).setNewAsnIds(request);
						} catch (Throwable t) {
							prtlnErr("AsnWithAttributesServiceHelper Error: " + t.getMessage());
							t.printStackTrace();
						}
					}
				}
				...
				// pre-process Attribute fields
				// prtln ("processing Element fields");
				for (Iterator i = im.getAttributeFields().iterator(); i.hasNext(); ) {
					InputField field = (InputField) i.next();
					try {
						docMap.put(field.getXPath(), field.getValue());
						// prtln ("\n" + field.getXPath() + "\n\t"  + field.getValue());
