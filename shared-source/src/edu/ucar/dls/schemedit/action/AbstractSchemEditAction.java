...
import edu.ucar.dls.schemedit.input.Msp2EditValidator;
import edu.ucar.dls.schemedit.input.SmileEditValidator;
import edu.ucar.dls.schemedit.input.OsmEditValidator;
import edu.ucar.dls.schemedit.input.AsnStdWithAttrsValidator;
import edu.ucar.dls.schemedit.input.SchemEditErrors;
import edu.ucar.dls.schemedit.input.SchemEditActionErrors;
import edu.ucar.dls.schemedit.input.UniqueValueChecker;
import edu.ucar.dls.schemedit.input.InputField;
...
public abstract class AbstractSchemEditAction extends DCSAction {
	...
	protected SchemEditValidator getValidator(ActionForm form, ActionMapping mapping, HttpServletRequest request) {
		...
		else if (xmlFormat.equals("comm_anno"))
			return new AsnStdWithAttrsValidator(sef, framework, mapping, request);
		else if (xmlFormat.equals("cow_item"))
			return new AsnStdWithAttrsValidator(sef, framework, mapping, request);
		else
	...
	public ActionForward execute(
		...
		if (this.readOnlyMode)
			errors.add("error", new ActionError("generic.error", "The system is in Read Only mode - editing is disabled"));
		if (!errors.isEmpty()) {
	...
	protected ActionErrors uniqueUrlCheck(ActionMapping mapping,
		...
		// hook to disable similar URL checking but retain dup checking
		String doSimilarUrlCheck = (String)this.servlet.getServletContext().getInitParameter("doSimilarUrlCheck");
		
		boolean doSimilarCheck = (doSimilarUrlCheck != null ? doSimilarUrlCheck.equals("true") : true);
		...
		try {
			...
			// remove self from dups
			if (dups != null) {
				for (int i = dups.size() - 1; i > -1; i--) {
					DupSim dupSim = (DupSim) dups.get(i);
