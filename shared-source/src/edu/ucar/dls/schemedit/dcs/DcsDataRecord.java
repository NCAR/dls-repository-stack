...
public class DcsDataRecord implements Serializable {
	...
	public DcsDataRecord(File dcsDataFile, MetaDataFramework framework, CollectionConfig collectionConfig, 
		DcsDataManager dcsDataManager) throws Exception {
	...
	public Date getLastTouchDateDate() {
		try {
			return SchemEditUtils.getFullDateFormat().parse(getLastTouchDate());
		} catch (Throwable t) {
	...
	/**
	 *  Gets the metadataHandle attribute of the DcsDataRecord object
	 *
	 * @return    The id value
	 */
/* 	public String getMetadataHandle() {
		return getNodeText("/dcsDataRecord/metadataHandle");
	} */


	/**
	 *  Sets the metadataHandle attribute of the DcsDataRecord object
	 *
	 * @param  id  The new id value
	 */
/* 	public void setMetadataHandle(String handle) {
		try {
			if (getNode("/dcsDataRecord/metadataHandle") == null) {
				this.getDocument().getRootElement().addElement("metadataHandle");
			}
			getNode("/dcsDataRecord/metadataHandle").setText(handle);
		} catch (Throwable e) {
			prtlnErr("setMetadataHandle failed: " + e.getMessage());
		}
	} */
	...
	// Collection Control Accessors
	String collectionInfoPath = "/dcsDataRecord/collectionInfo";
	
	/**  Clear the collectionInfo element  */
	public void clearCollectionInfo() {
		Element collectionInfo = getCollectionInfo();
		if (collectionInfo != null) {
			collectionInfo.clearContent();
		}
	}

	/**
	 *  Gets the collectionInfo attribute of the DcsDataRecord object
	 *
	 * @return    The collectionInfo value
	 */
	public Element getCollectionInfo() {
		return (Element) getNode(collectionInfoPath);
	}

	/**
	 *  Sets the collectionInfoValue attribute of the DcsDataRecord object
	 *
	 * @param  childName  The new collectionInfoValue value
	 * @param  value      The new collectionInfoValue value
	 */
	private void setCollectionInfoValue(String childName, String value) {
		Element collectionInfo = DocumentHelper.makeElement(this.doc, collectionInfoPath);
		Element child = DocumentHelper.makeElement(this.doc, collectionInfoPath + "/" + childName);
		child.setText(value);
	}


	String collectionMetadataHandlePath = "/dcsDataRecord/collectionInfo/metadataHandle";
	...
	public String getCollectionMetadataHandle() {
		String ret = getNodeText(collectionMetadataHandlePath);
		return ret;
	}
	 ...
	 *  Sets the metadataHandle attribute of the DcsDataRecord object
	 *
	 * @param  metadataHandle  The new metadataHandle value
	 */
	public void setCollectionMetadataHandle(String metadataHandle) {
		try {
			setCollectionInfoValue("metadataHandle", metadataHandle);
		} catch (Throwable e) {
			prtlnErr("setCollectionHandle failed: " + e.getMessage());
		}
	}

	String collectionInfoSetSpecPath = "/dcsDataRecord/collectionInfo/setSpec";
	...
	public String getSetSpec() {
		String ret = getNodeText(collectionInfoSetSpecPath);
		return ret;
	}
	...
	public void setSetSpec(String setSpec) {
		try {
			setCollectionInfoValue("setSpec", setSpec);
		} catch (Throwable e) {
			prtlnErr("setCollectionHandle failed: " + e.getMessage());
		}
	}
	...
	public String getNdrNormalizedStatus() {
		return (this.isFinalStatus() ? edu.ucar.dls.ndr.apiproxy.NDRConstants.NCS_FINAL_STATUS : this.getStatus());
	}
	...
	public Date getLastSyncDateDate() {
		try {
			return SchemEditUtils.getFullDateFormat().parse(getLastSyncDate());
		} catch (Throwable t) {
	...
	public Date getChangeDateDate() {
		try {
			return SchemEditUtils.getFullDateFormat().parse(getChangeDate());
		} catch (Throwable t) {
