...
import edu.ucar.dls.schemedit.standards.adn.DleseSuggestionServiceHelper;
import edu.ucar.dls.schemedit.standards.td.TeachersDomainServiceHelper;
...
public abstract class CATServiceHelper implements SuggestionServiceHelper {
	...
	public static CATServiceHelper getInstance(SchemEditForm sef)
		...
		try {
			...
			else if (sef.getXmlFormat().equals("lar")) {
				/* lar is another special case since the CAT UI is embedded in
				   a complex node, rather than implemented as a leaf
				*/
				helper = new LarSuggestionServiceHelper(sef, plugin);
			}
			...
			else if (sef.getXmlFormat().equals("comm_anno") ||
					 sef.getXmlFormat().equals("cow_item")) {
				helper = new AsnWithAttributesServiceHelper(sef, plugin);
			}
	...
	protected List getSelectedStandards(Document doc) {
		...
		if (nodes == null) {
			...
			// prtln ("\nInstanceDoc:" + edu.ucar.dls.xml.Dom4jUtils.prettyPrint(doc));
			return new ArrayList();
		}
	...
	public String getRecordUrl() {
		...
		try {
			...
			prtlnErr("getRecordUrl WARNING: url path not configured");
		}
		return url;
	}
			...
			for (int i = 0; i < selected.length; i++) {
				...
				if (node == null) {
					if (!(this.getStandardsManager() instanceof
						edu.ucar.dls.schemedit.standards.asn.AsnStandardsManager)) {
