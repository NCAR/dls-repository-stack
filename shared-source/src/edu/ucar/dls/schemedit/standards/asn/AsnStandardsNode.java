...
public class AsnStandardsNode implements StandardsNode {
	...
	public String getStatementNotation () {
		return this.std.getStatementNotation();
	}
	...
	public String getLabel() {
		if (this.label == null) {
			if (this.getLevel() == 1) {
				if (this.std.getAuthor().equals("NGSS"))
					this.label = this.getItemText() + " " + getLabelDecorator();
				else
					this.label = this.getItemText();
			}
			else if (this.getIsLeafNode()) {
				this.label = getLabelDecorator() + "  " + this.getItemText();
			}
			else {
				this.label = this.getItemText() + " " + getLabelDecorator();
			}
	...
	/** Returns bracketed gradeRange (e.g. "[ K-4]") by default, but returns "standardNotation"
	* for comm-core standards.
	*/
	private String getLabelDecorator () {
		String statementNotation = this.std.getStatementNotation();
		if (statementNotation != null && statementNotation.trim().length() > 0)
			return "[" + statementNotation + "]";
		else
			return "[" +  this.getGradeRange() + "]";
	}

	/**
	 *  The text of the standard as defined by ASN source document, appending comment in italics if
	 one is defined for this standard.
	 *
	 * @return    The itemText value
	 */
	public String getItemText() {
		String comment = std.getComment();
		if (std.isLeaf() && comment != null && comment.trim().length() > 0) {
			return std.getDescription() + " <i>" + comment + "</i>";
		}
		else
