 ...
 *  edu.ucar.dls.schemedit.autoform} classes as well as {@link
 *  edu.ucar.dls.schemedit.MetaDataFramework} instances.<p>
 *
 *  Note: why doesn't this class extend the {@link
 *  edu.ucar.dls.schemedit.config.AbstractConfigReader} class?
 *
 *@author    ostwald
 */
public class FieldInfoReader {
	...
	public FieldInfoReader(URI uri)
		...
		this.uri = uri;
		try {
			doc = SchemEditUtils.getRemoteDoc(uri.toString(), true);
			// prtln (Dom4jUtils.prettyPrint(doc));
		} catch (Exception e) {
			throw new Exception("getRemoteDoc error: " + e.getMessage());
		}
	}
	...
	*/
	...
	/**
	 *  Gets the {@link edu.ucar.dls.schemedit.vocab.TermAndDeftn} instances
	...
	public LabelValueBean[] getTermSelectOptions() {
		...
		options[0] = new LabelValueBean(edu.ucar.dls.schemedit.action.form.SchemEditForm.UNSPECIFIED, "");
		int index = 1;
	 ...
	 *  edu.ucar.dls.schemedit.vocab.TermAndDeft} instance.
	 *
	...
	private String getTextAtPath (String path) {
		try {
			...
			prtln ("Warning: could not get text a path: " + path);
		}
		return "";
	}
