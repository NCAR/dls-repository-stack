...
import edu.ucar.dls.index.reader.NewsOppsDocReader;
import edu.ucar.dls.index.reader.ItemDocReader;
...
public class DupSimUrlChecker {
	...
	private List getPrimaryDups() {
		...
		prtln ("  referenceURL is: " + this.referenceUrl);
		List dups = new ArrayList();
		...
		query.add(new TermQuery(new Term("collection", "0"+collection)), 
					 BooleanClause.Occur.MUST);
		query.add(new TermQuery(new Term("urlenc", urlenc)), 
					 BooleanClause.Occur.MUST);
		
		
		prtln("  dup url query: " + query.toString());
		...
		prtln(results.size() + " results (primary dups) found");
		if (results != null) {
	...
	private List getPrimarySims() {
		...
		BooleanQuery query = new BooleanQuery();
		query.add(new TermQuery(new Term("ky", collection)), 
					 BooleanClause.Occur.MUST);
 		query.add(new TermQuery(new Term("urlenc", simUrlEnc)), 
					 BooleanClause.Occur.MUST); // with SHOULD all docs are matched
		
		prtln ("Sims query: " + query.toString());
		
		// don't know why, but we get no results using Query query
		// ResultDocList results = index.searchDocs(query);		
		
		ResultDocList results = index.searchDocs(query.toString());
		
		List sims = new ArrayList();
		if (results != null) {
			prtln (results.size() + " found for " + SEARCH_LEVELS + "-level search");
	...
	private List getMirrorSims() {
		...
		String urlEnc = SimpleLuceneIndex.encodeToTerm(similarUrlPath);
		
		BooleanQuery query = new BooleanQuery();
		query.add(new TermQuery(new Term("collection", "0*")), 
					 BooleanClause.Occur.SHOULD);
		query.add(new TermQuery(new Term("collection", "0"+collection)), 
					 BooleanClause.Occur.SHOULD);
		query.add(new TermQuery(new Term("urlMirrorsEncoded", urlEnc)), 
					 BooleanClause.Occur.SHOULD);
