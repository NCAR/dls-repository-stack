...
public class RendererImpl extends Renderer {
	...
	*/
	...
	public Element getInputElement(String xpath, SchemaNode schemaNode, GlobalDef typeDef) {
		...
		if (schemaNode.isReadOnly()) 
			inputElement.addAttribute("madeBy", "getReadOnlyElement");
		/*
		...
		*/
		if (inputElement.getName().equals("table")) {
			...
			*/
			...
			if (schemaNode.isReadOnly()) {
				inputElement.addAttribute("id", "${id}-readonly");
			}
			else if (inputElementName.startsWith("html__")) {
	...
	/**
	 *  Gets a read-only version of input field for element specified by xpah
	 *
	 * @param  xpath  NOT YET DOCUMENTED
	...
	protected Element getReadOnlyElement(String xpath) {
		...
		Element text_wrapper = readOnlyElement.addElement("div")
								.addAttribute("class", "filename");
		Element text_content = text_wrapper.addElement("bean__write")
	...
	public SimpleTypeLabel getSimpleTypeLabel(String xpath, String siblingPath, String indexId) {
		...
		else if (sh.isRequiredContentElement(schemaNode, this.getXmlFormat())) {
		...
		if (getLevel(xpath) == 0) {
			...
			*/
			if ((sh.isRequiredBranch(schemaNode) ||
				sh.isRequiredContentElement(schemaNode, this.getXmlFormat())) &&
