	...
	private void attachControl(Element controlElement) {
		...
		prtln ("attach control");
 		if (hasRepeatingComplexSingletonChild()) {
			String repeatingChildPath = this.getRepeatingComplexSingletonChildPath();
			...
			prtln ("repeatingChildPath: " + repeatingChildPath);
			SchemaNode child = this.sh.getSchemaNode(RendererHelper.normalizeXPath(repeatingChildPath));
			
			if (child == null) {
				prtln ("WARNING: could not find child node for " + 
					RendererHelper.normalizeXPath(repeatingChildPath));
				return;
			}
		...
		for (Iterator i = children.iterator(); i.hasNext(); ) {
			...
			try {
				if (childType.equals("attribute")) {
					...
					/* getDiv is called in render, don't call it here */
					// Element subElementsBox = getDiv(); // subElementsBox
					Element subElementsBox = DocumentHelper.createElement("div");
					parent.add(subElementsBox);
...
*/
	...
	protected Element getComplexTypeBox() {
		...
		*/
		if (!sh.isRequiredContentElement(parentNode, this.renderer.getXmlFormat())) {
