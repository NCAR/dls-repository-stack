...
import edu.ucar.dls.schemedit.sif.SIFRefIdManager;
import edu.ucar.dls.util.Files;
import edu.ucar.dls.util.strings.FindAndReplace;
import edu.ucar.dls.standards.asn.util.AsnCatalog;
import edu.ucar.dls.standards.asn.AsnStandardsIdFieldPreprocessor;
...
import edu.ucar.dls.xml.nldr.OsmRecordExporter;
...
public final class SetupServlet extends HttpServlet {
	...
	public void init(ServletConfig config)
		...
		SchemEditUtils.disableSslVerification();
		prtln ("SSL Cert verification DISABLED");
		...
		// set up VirtualPageConfig instance (it is a singleton class) using the "virtualPageConfig" init param 
		String virtualPageConfigPath = (String) servletContext.getInitParameter("virtualPageConfig");
		if (virtualPageConfigPath != null && virtualPageConfigPath.trim().length() > 0) {
			try {
				VirtualPageConfig.setSourceURI (virtualPageConfigPath.trim());
				prtln ("virtualPageConfig sourceURI set to: " + virtualPageConfigPath);
			} catch (Exception e) {
				prtlnErr ("VirtualPageConfig ERROR: " + e);
			}
		}
		...
		// AsnResolverCache
		String asnResolverClientCache = (String) getServletContext().getInitParameter("asnClientCache");
		if (asnResolverClientCache != null && asnResolverClientCache.trim().length() > 0) {
			asnResolverClientCache = 
				GeneralServletTools.getAbsolutePath(asnResolverClientCache.trim(), getServletContext());
		}
		else {
			asnResolverClientCache = null;
		}
		prtln ("asnResolverClientCache: " + asnResolverClientCache);
		
		// AsnResolver
		String asnResolverBaseUrl = (String) getServletContext().getInitParameter("asnResolverBaseUrl");
		if (asnResolverClientCache != null && asnResolverBaseUrl != null && asnResolverBaseUrl.trim().length() > 0) {
			try {
				CachingAsnResolutionServiceClient asnResolutionServiceClient = 
					new CachingAsnResolutionServiceClient(asnResolverBaseUrl.trim(), asnResolverClientCache);
				...
				if (AsnStandardsIdFieldPreprocessor.getAsnResolutionServiceClient() == null)
					throw new Exception ("client has not been properly stowed in AsnStandardsIdFieldPreprocessor");
			} catch (Throwable t) {
		...
		if (nldrPropsPath != null && nldrPropsPath.trim().length() > 0) {
			try {
				this.nldrSetup (nldrPropsPath);
			} catch (Exception e) {
				prtlnErr ("NLDR Setup ERROR: " + e.getMessage());
				e.printStackTrace();
			}
		}
	...
	void loginModuleSetUp() {
		...
		if (AuthUtils.loginModuleEnabled("edu.ucar.dls.schemedit.security.login.FileLogin")) {
			String passwordFile = AuthUtils.getPasswordFile();
			if (passwordFile != null) {
				edu.ucar.dls.schemedit.security.login.PasswordHelper.getInstance(passwordFile);
	...
	/**
	* Process NLDR properties and initialize the threaded service that updates recordsAffected field
	*/
	void nldrSetup (String nldrPropsPath) throws ServletException  {
		ServletContext servletContext = getServletContext();
		...
		try {
			prtln ("nldrPropsPath: " + nldrPropsPath);
			NLDRProperties nldrProperties = new NLDRProperties (nldrPropsPath);
			OsmRecordExporter.setNLDRProperties (nldrProperties);
			servletContext.setAttribute ("nldrProperties", nldrProperties);
		} catch (Exception e) {
			throw new ServletException ("could not instantiate NLDRProperties for " 
										+ nldrPropsPath + ": " + e.getMessage());
		}
		prtln ("props set for OsmRecordExporter");
		
		// initialize vocabUpdater
		String repositoryData = getAbsolutePath((String) servletContext.getInitParameter("repositoryData"));

		VocabRecordsAffectedService vocabRecordsAffectedService = new VocabRecordsAffectedService(servletContext, repositoryData + "/vocab_affected_records_service");
		servletContext.setAttribute("vocabRecordsAffectedService", vocabRecordsAffectedService);
		prtln ("set vocabRecordsAffectedService in servlet context");
	...
	void standardsServiceSetup() throws ServletException {
		...
		String cachePath = getAbsolutePath("WEB-INF/data/ASN-Catalog-Cache/topics");
		if (! (new File (cachePath).exists()))
			throw new ServletException ("ASN cachePath  does not exist at " + cachePath);
		AsnCatalog.setCacheDirectory(cachePath);
		prtln ("setAsnCatalog cache directory to: " + cachePath);
		...
		getServletContext().setAttribute("standardsRegistry", StandardsRegistry.getInstance());
		prtln("placed standardsRegistry in servlet context");
	...
	public static void setLogging() {
		...
		edu.ucar.dls.xml.schema.StructureWalker.setDebug(false);
		edu.ucar.dls.xml.schema.SchemaReader.setDebug(false);
		edu.ucar.dls.xml.schema.SchemaHelper.setVerbose(false);
		edu.ucar.dls.xml.schema.DefinitionMiner.setDebug(false);
	}
