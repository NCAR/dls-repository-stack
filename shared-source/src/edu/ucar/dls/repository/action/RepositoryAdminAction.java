			...
			if (command != null) {
				...
				if (command.equals("indexingManagerAction")) {
					...
					if (indexingManager == null) {
						String error = null;
						if(((Maintenance)servletContext.getAttribute("repositoryMaintenance")).isIndexDisabledDueToActionRequired())
							error = "Index is temporarily disabled due to an action required condition. View Repository Maintenance to resolve action.";
						else
							error = "Can not index from external source - no IndexingManager is configured in the system";
						errors.add("error", new ActionError("generic.error", error));
						saveErrors(req, errors);
						return mapping.findForward("display.repository.settings");
					}
			...
			if (req.getParameter("currentSetSpec") != null) {
				...
				if (req.getParameter("add") != null) {
					if (!req.getParameter("currentSetSpec").equals("")) {
						...
						String dir = null, format = null, id = null;

						SetInfo set = new SetInfo(
							req.getParameter("currentSetName").trim(),
							req.getParameter("currentSetSpec").trim(),
							currentSetDescription,
							enableNewSets, dir, format, id);
