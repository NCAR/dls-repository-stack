...
import edu.ucar.dls.services.dds.action.DDSServicesAction;
 ...
 * @version    $Id: RepositoryAction.java,v 1.43 2012/04/13 15:53:14 jweather Exp $
 */
public final class RepositoryAction extends Action {
	...
	protected ActionForward doGetRecord(
		...
		if (identifier != null && identifier.length() > 0 && format != null) {
			...
			// Query for a result:
			ResultDoc resultDoc = rm.getRecordOai(idStripped,request.getParameterValues("disableFilter"));
	...
	protected ActionForward doListRecordsOrIdentifiers(
		...
		if (set != null && set.startsWith("dleseodlsearch")) {
			...
			if (!rf.hasErrors() && odl != null) {
				try {
					results = rm.getOdlQueryResults(format, odl.getSet(), from, until, odl.getQueryString(),request.getParameterValues("disableFilter"));
				} catch (Exception e) {
		...
		// -------  Handle standard OAI-PMH ---------
			...
			if (!rf.hasErrors() && rth != null) {
				try {
					results = rm.getOaiQueryResults(rth.getFormat(), rth.getSet(), rth.getFrom(), rth.getUntil(), request.getParameterValues("disableFilter"));
					//if (results != null)
	...
	private boolean validateRequestArguments(HttpServletRequest request, RepositoryForm rf) {
		...
		if (validArgsMap == null) {
			...
			// Allow the disableFilter argument, used to allow otherwise filtered records through
			validArgsMap.put("disableFilter", "disableFilter");		
		}
