	...
	private void processRecords(String xslFile,
		...
		if (validateInputFiles == true) {
			...
			try {
				...
				reportTxt = validator.validate(inputFilesDirFull,enableSchemaCaching);
		...
		if (validateOutputFiles == true) {
			...
			try {
				XMLValidator validator = new XMLValidator();
				boolean enableSchemaCaching = true;
				reportTxt = validator.validate(outputDirFull,enableSchemaCaching);
