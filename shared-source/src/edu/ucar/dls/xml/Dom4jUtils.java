 ...
 *
 *
 * @version    $Id: Dom4jUtils.java,v 1.24 2016/03/01 00:20:34 jweather Exp $
 */
public class Dom4jUtils {
	...
	public static Document getXmlDocument(URL url, int timoutMs, String userAgent)
		...
		try
		{
			inputStream = TimedURLConnection.getInputStream(url, timoutMs, userAgent);
			// No validation...
			SAXReader reader = new SAXReader(false);
	...
	/**
	 *  Load XML into a dom4j Document, specifying a max time to wait for response and other arguments. Supports
	...
	public static Document getXmlDocumentPostData(URL url, String postData, int timoutMs, String userAgent)
		...
		try
		{
			inputStream = TimedURLConnection.getInputStream(url, postData, timoutMs, userAgent);
			// No validation...
			SAXReader reader = new SAXReader(false);
			Document document = reader.read(inputStream);
			return document;
	...
	/**
	 *  Load XML into a dom4j Document and localize the XML by removing all namespaces from it. If error, return
	 *  null. Same as {@link #getXmlDocumentLocalized(String)} except all Exceptions are thrown if encountered.
	 *
	 * @param  xml                    A string representation of an XML document
	 * @return                        An XML Document containing the localized dom4j DOM, or null if unable to
	 *      process the string.
	 ...
	 */
	public static Document getXmlDocumentLocalizedThrowsExceptions(String xml) throws Exception, DocumentException {
		// No validation...
		SAXReader reader = new SAXReader(false);
		StringReader sr = new StringReader(localizeXmlThrowsExceptions(xml));
		Document document = reader.read(sr);
		sr.close();
		return document;
	}
	...
	public static Object dom4j2JavaBean(Node javaObjectXmlNode) throws Exception {
		ByteArrayInputStream bim = null;
		BufferedInputStream bis = null;
		XMLDecoder d = null;
		
		try
		{
			bim = new ByteArrayInputStream(javaObjectXmlNode.asXML().getBytes("UTF-8"));
			bis = new BufferedInputStream(bim);
	...
	/**
	 *  Localizes an XML String by removing all namespaces from it. Same as {@link #localizeXml(String)} except
	 ...
	 * @exception  Exception  If error
	 */
	public static String localizeXmlThrowsExceptions(String xml) throws Exception {
		javax.xml.transform.Transformer transformer = null;
		transformer = getLocalizingTransformer();
		return XSLTransformer.transformStringThrowsExceptions(xml, transformer);
	}
