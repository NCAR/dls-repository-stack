...
import edu.ucar.dls.schemedit.SchemEditUtils;  // i don't like this dependence!!
...
public class DefinitionMiner {
	...
	private Document getParsedDoc(URI uri)
		...
		if (parsedDocs.containsKey(uri.toString())) {
			...
			if (uri.getScheme().equals("file")) {
				...
				String msg = "";
				try {
					doc = SchemEditUtils.getRemoteDoc (uri.toString(), false);
				} catch (Exception e) {
					msg = "getRemoteDoc ERROR: " + e;
					prtln (msg);
				...
				throw new SchemaHelperException ("Unknown URI scheme: " + uri.getScheme());
			}
			// prtln ("parsed " + uri.toString() + "\n");
	...
	protected void processSchemaFile(URI uri, String defaultTargetNamespaceURI) throws SchemaHelperException {
		...
		try {
			...
			if (doc == null)
				throw new Exception ("DefinitionMiner.getParsedDoc() returned NULL document");
		} catch (Exception e) {
