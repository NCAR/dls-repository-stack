 ...
 *  turn encapsulated in the {@link edu.ucar.dls.schemedit.MetaDataFramework}
 ...
 *    <li> {@link edu.ucar.dls.xml.schema.XSDatatypeManager} for performing
 *    iterative validation of xml documents using datatypes defined in the
 *    Schema.</li>
 *    <li> {@link edu.ucar.dls.xml.schema.DefinitionMiner} which traverses an
 *    XML Schema and builds a dictionary of {@link edu.ucar.dls.xml.schema.GlobalElement}
 *    objects, one for each element defined in the Schema.</li>
 *    <li> {@link edu.ucar.dls.xml.schema.SchemaNodeMap} that represents the
 *    structure defined by the Schema as a tree of {@link edu.ucar.dls.xml.schema.SchemaNode}
 *    objects.</li>
 *  </ul>
...
public class SchemaHelper {
	...
	/**
	 *  Gets the schemaNamespace attribute of the SchemaHelper object
	 *
	 ...
	 *
	 * @param  schemaNode  a {@link edu.ucar.dls.xml.schema.SchemaNode} object
	 * @return             a {@link edu.ucar.dls.xml.schema.GlobalDef} object, or
	 *      null if not found.
	 */
	 ...
	 *  EditorAction in the "suggest" package edu.ucar.dls.to validate the email
	...
	public boolean isRepeatingElement(SchemaNode schemaNode, GlobalDef typeDef) {
		...
		if (schemaNode.isSubstitutionGroupMember()) {
			String headName = schemaNode.getHeadElementName();
			SchemaNode headNode = schemaNode.getSubstitutionGroupHeadNode();
			if (headNode == null) {
				prtln("headNode not found");
				return false;
			}
			else {
				prtln("headnode is repeatable? " + this.isRepeatingElement(headNode));
				return this.isRepeatingElement(headNode);
			}
		}
	...
	/**
	 *  Gets the recursiveElement attribute of the SchemaHelper object
	 *
	 * @param  encodedXPath  NOT YET DOCUMENTED
	 * @return               The recursiveElement value
	 */
	...
	/**
	 *  Gets the anyTypeElement attribute of the SchemaHelper object
	 *
	 * @param  xpath  NOT YET DOCUMENTED
	 * @return        The anyTypeElement value
	 */
	...
	/**
	 *  Gets the requiredContentElement attribute of the SchemaHelper object
	 *
	 * @param  schemaNode  NOT YET DOCUMENTED
	 * @return             The requiredContentElement value
	 */
	public boolean isRequiredContentElement(SchemaNode schemaNode) {
		return isRequiredContentElement(schemaNode, null);
	}
	 ...
	 *      have content
	 */
	public boolean isRequiredContentElement(SchemaNode schemaNode, String xmlFormat) {
		...
		if (schemaNode.isElement()) {
			...
			if (typeDef.isComplexType()) {
				...
				if (complexTypeDef.isDerivedType()) {
					...
					// end run the KLUDGE if framework is lar
					if (xmlFormat != null && xmlFormat.equals("lar"))
						return schemaNode.isRequired();
	...
	/**
	 *  Returns true if a {@link edu.ucar.dls.xml.schema.SchemaNode} defines a
	 *  required attribute.
	 *
	 * @param  schemaNode  Description of the Parameter
 ...
 */
	...
	/**
	 *  Returns a list of attribute names that can appear in an instanceDoc for
	 *  Element at specified path.
	 *
	 * @param  xpath  path to an Element for this framework.
	 * @return        attribute names that can appear for the specified Element
	 */
	public List getAttributeNames(String xpath) {
		String normalizedXPath = XPathUtils.normalizeXPath(xpath);
		List list = new ArrayList();
		Node node = getInstanceDocNode(normalizedXPath);
		...
		if (node != null) {
			if (node.getNodeType() != Node.ELEMENT_NODE) {
				prtln("node at " + xpath + " is note an element");
				return list;
			}
	...
	*/
	 ...
	 *  the globalDef NAME from the {@link edu.ucar.dls.xml.schema.SchemaNodeMap}
	 *  and then using the name as an index into the {@link edu.ucar.dls.xml.schema.GlobalDefMap}.
	 *
	 * @param  xpath  an xpath to a specific node
	 * @return        an {@link edu.ucar.dls.xml.schema.GlobalDef} object
	 */
...
*/
	...
	public Element getNewElement(String encodedPath) {
		...
		if (schemaNode.isSubstitutionGroupMember()) {
			prtln("schemaNode.isSubstitutionGroupMember");
			...
			prtln("schemaNode.is NOT SubstitutionGroupMember");
			Node node = doc.selectSingleNode(xpath);
			if (node == null) {
				prtln("getNewElement failed to find instanceDocument node at " + xpath);
				if (this.getSchemaNode(xpath) != null) {
					prtln("creating simple element: " + XPathUtils.getNodeName(xpath));
					return DocumentHelper.createElement(XPathUtils.getNodeName(xpath));
				}
				return null;
			}
...
*/
	 ...
	 *  version that is known to the schema. I.e., Replaces the node-name of '*'
	 *  with 'any' while preserving any indexing.
	 *
	 * @param  xpath  NOT YET DOCUMENTED
	 * @return        NOT YET DOCUMENTED
	 */
