 ...
 *  edu.ucar.dls.xml.schema.SchemaNodeMap} that is keyed by XPaths.
 *
 *@author    ostwald
 */
public class SchemaNode {
	...
	*/
	...
	/**
		A substitution group comprises the typeDefs (as GlobalElements) of global schema elements that
		declare the type represented by this SchemaNode as their substitutionGroup attribute.</p>
		
		E.g., <xs:element name="person" type="authorContactBaseType" substitutionGroup="contributor" /></p>
		
		A schemaNode with a non-empty substitutionGroup is abstract and a headElement
	*/
	...
	*/
	...
	/* the name of the substitution group of which this schemaNode is
	a member.
	*/
	public String getSubstitutionGroupHead () {
		String head = getAttr("substitutionGroup").trim();
		if ("".equals(head))
			return null;
		return head;
	}
	...
	/**
	* gets the schemaNode for the head of this schenaNode's substitution group.
	The schemaNode represents an abstract element.
	*/
	public SchemaNode getSubstitutionGroupHeadNode () {
		String head = getSubstitutionGroupHead();
		if ("".equals(head))
			return null;
		String xpath = XPathUtils.getParentXPath(this.xpath) + "/" + head;
		return (SchemaNode)this.schemaNodeMap.getValue(xpath);
	}
