...
/**
 *  Manages the conversion of XML files from one format to another using XSL or Java classes. Caches the
 *  converted XML to disc and provides rapid access to the converted format. Conversions may be accomplished
 *  using XSL stylesheets or Java classes that implement one of either the {@link XMLFormatConverter} or
 *  {@link XMLDocumentFormatConverter} interfaces.
 *
 * @author     John Weatherley
 * @version    $Id: XMLConversionService.java,v 1.34 2013/08/29 22:44:35 finkedave Exp $
...
public final class XMLConversionService {
	 ...
	 *
	 * @param  cacheDir            The directory where converted files will be cached and stored for later
	 *      retrieval by ID.
	 * @param  filterDeclarations  Set to true to filter out the XML and DTD declarations in the converted XML.
	 * @exception  IOException     If the directory is not valid or does not have read/write access.
	 */
	...
	/**
	 *  Converts XML from one format to another, saving and retrieving the converted content to and from a file
	 *  cache. Returns null if there are no known conversions available for the requested conversion, or if a
	 *  processing error occurs. The cache file is automatically updated if the modification time of the input
	 *  XML file is newer than the cache. Characters returned in the StringBuffer are encoded in UTF-8.
	 *
	...
	/**
	 *  Converts XML from one format to another, without the use of a cache to store or save results. Returns
	 *  null if there are no known conversions available for the requested conversion, or if a processing error
	 *  occurs. Characters returned in the StringBuffer are encoded in UTF-8.
	 *
	 * @param  fromFormat   The XML format to convert from. Example: 'dlese_ims.'
	 * @param  toFormat     The format to convert to. Example: 'adn.'
	 * @param  originalXML  The original XML as a String, in the 'from' format. Should not be null.
	 * @return              Content converted to the 'to' format, or null if unable to process.
	 */
	...
	/**
	 * Custom destroy method that tries to destroy all the converter objects
	 */
	public void destroy()
	{
		for(Object convertObj: this.converters.values())
		{
			Hashtable convertHashtable = (Hashtable)convertObj;
			for(Object xmlConverterObj: convertHashtable.values())
			{
				((XmlConverter)xmlConverterObj).destroy();
			}
		}
	}
	 ...
	 * @param  fromFormat       The XML format to convert from. Example: 'dlese_ims.'
	 * @param  toFormat         The format to convert to. Example: 'adn.'
	 * @param  originalXMLFile  The original XML file, in the 'from' format. Should not be null.
	 * @param  luceneDoc        A Lucene {@link org.apache.lucene.document.Document} that holds content that may
	 *      be used by the conversion class, or null.
	 * @return                  Content converted to the 'to' format, or null if unable to process.
	 */
	public StringBuffer getConvertedXml(String fromFormat,
			String toFormat,
	...
	/**
	 *  Determines whether this XMLConversionService can perform the given converstion. Returns true of the
	 *  toFormat is the same as the fromFormat.
	 *
	...
	/**
	 *  Gets the converter that is available to convert from one format to another. Returns null if no converter
	 *  is available.
	 *
	 * @param  fromFormat  The format to convert from.
	 * @param  toFormat    The format to convert to.
	...
	/**
	 *  Adds a concrete implementation of the {@link XMLFormatConverter} interface that can convert XML from one
	 *  format to another. The class must be available in the classpath for Tomcat or the running JVM that is
	 *  using this broker.
	 *
	 ...
	 * @param  className       The fully-qualified class that will perform the conversion.
	 * @param  servletContext  The feature to be added to the JavaConverterClass attribute
	 */
	...
	/**
	 *  Adds an XSL converter by parsing a String of the form [ fromFormat | toFormat | xslFileName ]. The value
	 *  'disabled' is also accepted (does nothing). Useful for using initParameters to configure XML format
	 *  converters.
	 *
	 * @param  paramVal              A String of the form [ fromFormat | toFormat | xslFileName ] or 'disabled'.
	 * @param  xslFilesDirecoryPath  The full path to the directory where the xsl file exists.
	 * @return                       False if the number of values parsed in paramVal is incorrect, otherwise
	 *      true.
	 */
	public boolean addXslConverterHelper(String paramVal, File xslFilesDirecoryPath) {
		String[] vals = paramVal.split("\\|");
		if (vals.length != 3) {
			if (!paramVal.equals("disabled"))
				prtlnErr("addXslConverterHelper() error: could not parse parameter '" + paramVal + "'");
			return false;
		}
	...
	/**
	 *  Adds an Java class XML converter by parsing a String of the form [ fromFormat | toFormat | className ]. The value
	 *  'disabled' is also accepted (does nothing).
	 *  Useful for using initParameters to configure XML format converters.
	 *
	 * @param  paramVal        A String of the form [ fromFormat | toFormat | xslFileName ] of 'disabled'.
	 * @param  servletContext  The ServletContext that will be available to the class when it executes.
	 * @return                 False if the number of values parsed in paramVal is incorrect, otherwise true.
	 */
	public boolean addJavaConverterHelper(String paramVal, ServletContext servletContext) {
		...
		if (vals.length != 3) {
			if (!paramVal.equals("disabled"))
				prtlnErr("addJavaConverterHelper() error: could not parse parameter '" + paramVal + "'");
			return false;
		}
	...
	/**
	 *  Factory for use in Servlets to create an XMLConversionService using configurations found in the Servlet
	 *  context parameters. This method also places the path to the XSL files directory in the servlet context
	 *  under 'xslFilesDirecoryPath', for use by the JavaXmlConverters. <p>
	 *
	 *  To configure a format converter that uses an XSL style sheet or Java class, the context param-name must
	 *  begin with the either 'xslconverter' or 'javaconverter' and must be unique. The param-value must be of
	 *  the form 'xslfile.xsl|[from format]\[to format]' or '[fully qualified Java class]|[from format]\[to
	 *  format]'.
	 *
	 * @param  servletContext      The ServletContext
	 * @param  xslFilesDirecory    The base directory where the XSL files are located
	 * @param  xmlCachDirecory     The directory where the service will cache the converted XML files
	 * @param  filterDeclarations  Set to true to filter out the XML and DTD declarations in the converted XML
	 ...
	 */
	public final static StringBuffer stripXmlDeclaration(BufferedReader rdr)
			 throws IOException {
	...
	/**
	 *  Gets the content from XML by stripping all XML tags. The input XML should be valid prior to calling this
	 *  method.
	 *
	...
	private interface XmlConverter {
		 ...
		 * @param  fileToWrite      The file that must be written to with the converted output.
		...
		/**
		 * Destroy method, that enables the converters to be able to clean up their resources
		 */
		public void destroy();
		...
		/**
		 *  Gets the time this converter code was last modified. If unknown, this method should return -1 to
		 *  indicate updates should occur only when the source XML has change or System.currentTimeMillis() + 100000
		 *  to indicate updates should occur upon every request for a conversion.
		 *
	...
	private class XslConverter implements XmlConverter {
		...
		/**
		 * Unused destroy method for xsl converters that later can be implemented
		 */
		public void destroy() {
	...
	private class JavaXmlConverter implements XmlConverter {
		 ...
		 *
		 * @param  originalXML      Description of the Parameter
		...
		/**
		 * Destroy all format converters for Java converters
		 */
		public void destroy() {
			if (xmlDocumentFormatConverter != null) {
				xmlDocumentFormatConverter.destroy();
			}
			if (xmlFormatConverter != null) {
				xmlFormatConverter.destroy();	
			}
