 ...
 * @version    $Id: XSLTransformer.java,v 1.21 2012/01/27 00:36:30 jweather Exp $
 */
public class XSLTransformer {
	 ...
	 *  #getTransformer(String xslFilePath)} to produce a reusable {@link javax.xml.transform.Transformer} for a
	 *  given XSL stylesheet. Returns empty String if Excpeptions are encountered.
	 *
	 ...
	 *  Transforms an XML String using a pre-compiled {@link javax.xml.transform.Transformer}. Use {@link
	 *  #getTransformer(String xslFilePath)} to produce a reusable {@link javax.xml.transform.Transformer} for a
	 *  given XSL stylesheet. Throws Exceptions if encountered.
	 *
	 * @param  xmlString      The XML String to transform.
	 * @param  transformer    A pre-compiled {@link javax.xml.transform.Transformer} used to produce transformed
	 *      output.
	 ...
	 */
	public final static String transformStringThrowsExceptions(String xmlString, Transformer transformer) throws Exception {
		StringWriter writer = new StringWriter();
		StreamSource source = new StreamSource(new StringReader(xmlString));
		transformer.transform(source, new StreamResult(writer));
		return writer.toString();
	...
	/**
	 *  Transforms an XML String using an XSL stylesheet supplied as a String, and returns empty string if
	...
	/**
	 *  Transforms an XML String using an XSL stylesheet supplied as a String, and throws Exceptions if
	 *  encountered;
	 *
	 * @param  xmlString      The XML String to transform.
	 * @param  xslString      The XSL String used to define the transform.
	 ...
	 * @exception  Exception  If error occurs
	 */
	public final static String transformStringUsingStringThrowsExceptions(String xmlString, String xslString) throws Exception {
		return transformStringThrowsExceptions(xmlString, getTransformerFromXSLString(xslString));
	}


	/**
	 *  Removes all namespace information from XML and returns empty string if Exception is encountered.
	 *
	...
	/**
	 *  Removes all namespace information from XML and throws Exceptions if encountered.
	 *
	 * @param  xmlString      The XML String to strip of namespaces
	 * @return                A String containing XML without namespaces
	 * @exception  Exception  If error occurs
	 * @see                   #XSLUtils.getRemoveNamespacesXSL()
	 */
	public final static String localizeXmlThrowsExceptions(String xmlString) throws Exception {
		return transformStringUsingStringThrowsExceptions(xmlString, XSLUtils.getRemoveNamespacesXSL());
	}
	 ...
	 *  single stylesheet.
	 *
	 * @param  xslFilePath                            A path to an XSL stylesheet file.
	...
	/**
	 *  Gets a specific {@link javax.xml.transform.Transformer}, used to transform XML using a given XSL
	 *  stylesheet.
	 *
	 * @param  xslFilePath                            A path to an XSL stylesheet file.
	 * @param  transformerFactoryClass                TransformerFactory class (e.g.
	...
	/**
	 *  Gets a transformerFactory instance using a specified TransformerFactory class, or the default
	 *  TransformerFactory class if "tFactoryClass" in null.
	 *
