...
<c:set var="ddswsBaseUrl" value="${ddsws11BaseUrl}"/>
...
<c:if test="${param.reload == 'true'}">
	...
	<c:remove var="gradeRangesXmlDom_DDSExpv11" scope="application"/>
	<c:remove var="subjectsXmlDom_DDSExpv11" scope="application"/>
	<c:remove var="resourceTypesXmlDom_DDSExpv11" scope="application"/>
	<c:remove var="collectionsXmlDom_DDSExpv11" scope="application"/>	
	<c:remove var="contentStandardsXmlDom_DDSExpv11" scope="application"/>
</c:if>
...
<c:if test="${not empty param.fullDescription || not empty param.collectionDescription}">
	...
	<c:catch var="getRecordXmlDomError">
		...
		<x:transform xslt="${f:removeNamespacesXsl()}" xml="${xmlResponse}" var="getRecordXmlDom"/>
	</c:catch>	
</c:if>
...
<c:if test="${empty gradeRangesXmlDom_DDSExpv11}">
	<c:catch var="grError">			
		...
		<%-- Store and cache the grade ranges in an application scope XML DOM variable 'gradeRangesXmlDom_DDSExpv11' --%>				
		<x:transform xslt="${f:removeNamespacesXsl()}" xml="${xmlOutput}" var="gradeRangesXmlDom_DDSExpv11" scope="application"/>
...
<%-- Request and cache the subjects as an application scoped variable
        only if they have not been cached already --%>								
<c:if test="${empty subjectsXmlDom_DDSExpv11}">
	<c:catch var="suError">			
		<%-- Construct the request (URL) --%>
		<c:url value="${ddswsBaseUrl}" var="request">
		...
		<%-- Store and cache the subjects in an application scope XML DOM variable 'subjectsXmlDom_DDSExpv11' --%>				
		<x:transform xslt="${f:removeNamespacesXsl()}" xml="${xmlOutput}" var="subjectsXmlDom_DDSExpv11" scope="application"/>
	</c:catch>
...
<%-- Request and cache the resource types as an application scoped variable
        only if they have not been cached already --%>								
<c:if test="${empty resourceTypesXmlDom_DDSExpv11}">
	<c:catch var="reError">			
		<%-- Construct the request (URL) --%>
		<c:url value="${ddswsBaseUrl}" var="request">
		...
		<%-- Store and cache the resource types in an application scope XML DOM variable 'resourceTypesXmlDom_DDSExpv11' --%>				
		<x:transform xslt="${f:removeNamespacesXsl()}" xml="${xmlOutput}" var="resourceTypesXmlDom_DDSExpv11" scope="application"/>
	</c:catch>
...
<%-- Request and cache the collections as an application scoped variable
        only if they have not been cached already --%>								
<c:if test="${empty collectionsXmlDom_DDSExpv11}">
	<c:catch var="kyError">			
		<%-- Construct the request (URL) --%>
		<c:url value="${ddswsBaseUrl}" var="request">
		...
		<%-- Store and cache the resource types in an application scope XML DOM variable 'collectionsXmlDom_DDSExpv11' --%>				
		<x:transform xslt="${f:removeNamespacesXsl()}" xml="${xmlOutput}" var="collectionsXmlDom_DDSExpv11" scope="application"/>
	</c:catch>
...
<%-- Request and cache the content standards as an application scoped variable
        only if they have not been cached already --%>								
<c:if test="${empty contentStandardsXmlDom_DDSExpv11}">
	<c:catch var="csError">			
		<%-- Construct the request (URL) --%>
		<c:url value="${ddswsBaseUrl}" var="request">
		...
		<%-- Store and cache the resource types in an application scope XML DOM variable 'contentStandardsXmlDom_DDSExpv11' --%>				
		<x:transform xslt="${f:removeNamespacesXsl()}" xml="${xmlOutput}" var="contentStandardsXmlDom_DDSExpv11" scope="application"/>
	</c:catch>
