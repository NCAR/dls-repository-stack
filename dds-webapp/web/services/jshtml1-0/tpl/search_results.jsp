...
<c:choose>
	...
	<c:otherwise>	
		...
		<c:if test="${not empty param.q}">										
			...
			<c:set var="usersText">
				<span style="color:${keywordHighlightColor}">'<b>${fn:trim( param.q )}</b>'</span>
			</c:set>
		</c:if> 						
		...
		<table width="100%" cellpadding="4" cellspacing="0">			
			<tr>
				<td colspan="2">
					...
					<table bgcolor="#777777" width="100%" cellpadding="0" cellspacing="1" border="0" class="resultSectionHeading">
						<tr valign="center">
							<td>
								<c:choose>
									<c:when test="${not empty param.d && param.allrecs != 'n'}">
										<div class="resultSectionHeading">Educational resources</div> 
									</c:when>
									<c:otherwise>
										<div class="resultSectionHeading">Search results</div>
									</c:otherwise>
								</c:choose>
							</td>
						</tr>
			...
			<c:if test="${not empty param.d && param.allrecs != 'n'}">
				<tr>
					<td colspan="2">
						<span class="cusTitle">${param.d}</span>
					</td>
				</tr>				
			</c:if>				
			<tr>
			...
			<c:choose>
				...
				<c:otherwise>
					<c:if test="${empty param.d && param.allrecs != 'n'}">
						<tr>
							<td colspan="2">
			...
			<c:choose> 
				...
				<x:forEach select="$searchXmlDom/DDSWebService/Search/results/record" varStatus="status" var="record">
				<tr>
					<td colspan="2">
						...
						<c:set var="redirectURL">
							<c:choose>
								<c:when test="${not empty redirectBaseUrl}">
									${redirectBaseUrl}/T=search&ID=<x:out select="$record/head/id"/>&Q=${f:URLEncoder(param.q)}${f:URLEncoder(param.qh)}&R=${startingOffset + status.index}&SC=${clientName}*${resourceURL}
								</c:when>
								<c:otherwise>
									${resourceURL}
								</c:otherwise>
					...
					<td colspan="2">
						<div class="description">
							...
							<c:url value="" var="fullDescUrl">
								<c:param name="fullDescription"><x:out select="head/id"/></c:param>
								
								<%-- Echo each of the params the user has selected --%>
								<c:forTokens var="paramName" items="q qh s d ${vocabParameterNames} ${smartLinkMenuParameterNames}" delims=" ">
									<c:forEach var="paramValue" items="${paramValues[paramName]}">
										<c:param name="${paramName}" value="${paramValue}"/>
									</c:forEach>
								</c:forTokens>											
							</c:url>								
							<nobr><a href="${fullDescUrl}">Full description</a></nobr>
							
							<div style="padding-left:18px">
								<%-- Display the Grade level --%>
								<div style="padding-top:4px">
									<i>Grade level:</i>
									<x:forEach select="$record/metadata/itemRecord/educational/audiences/audience/gradeRange" varStatus="i">
										<c:set var="gradeRangeVocabEntry">
											<x:out select="."/>
										</c:set>									
										<x:out select="$gradeRangesElement/gradeRange[vocabEntry=$gradeRangeVocabEntry]/renderingGuidelines/label"/>${i.last ? '' : ','}		
									</x:forEach>
								</div>	
								...
								<div style="padding-top:2px">
									...
									<x:forEach select="$record/metadata/itemRecord/educational/resourceTypes/resourceType" varStatus="i">
										<c:set var="resourceTypeVocabEntry">
											<x:out select="."/>
										</c:set>														
										<dt:keywordsHighlight>
											<x:out select="$resourceTypesElement/resourceType[vocabEntry=$resourceTypeVocabEntry]/renderingGuidelines/label"/>${i.last ? '' : ','}
										</dt:keywordsHighlight>													
									</x:forEach>
								</div>							
			...
			<c:otherwise>
				<tr>
					<td colspan="2" style="padding-top:24px; padding-bottom:20px;">				
					<c:choose>
						<c:when test="${not empty param.d && param.allrecs != 'n'}">
