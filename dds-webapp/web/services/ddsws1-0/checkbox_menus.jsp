...
<c:if test="${param.reload == 'true'}">
	<c:remove var="menusLoaded_DDSExp" scope="application"/>
</c:if>
...
<c:if test="${ menusLoaded_DDSExp != 'true' }">

	<%-- Create the grade range menu and cache it for efficiency --%>
	<c:set var="currentIndent" value="0"/>
	<c:set var="gradeRangesCheckBoxMenu_DDSExp" scope="application">
		<div>
		...
		<x:forEach select="$gradeRangesXmlDom_DDSExp/DDSWebService/ListGradeRanges/gradeRanges/gradeRange">
	...
	<c:set var="subjectsCheckBoxMenu_DDSExp" scope="application">			
 		<div>
		...
		<x:forEach select="$subjectsXmlDom_DDSExp/DDSWebService/ListSubjects/subjects/subject" varStatus="status">
			<x:if select="contains( renderingGuidelines/noDisplay, 'false' )">
	...
	<c:set var="resourceTypesCheckBoxMenu_DDSExp" scope="application">	
 		<div>
		...
		<x:forEach select="$resourceTypesXmlDom_DDSExp/DDSWebService/ListResourceTypes/resourceTypes/resourceType" varStatus="status">
			<x:if select="contains( renderingGuidelines/noDisplay, 'false' )">
				<x:choose>
					<%-- Render the sub-list headings --%>
					<x:when select="contains( renderingGuidelines/hasSubList, 'true' )">					
	...
	<c:set var="contentStandardsCheckBoxMenuDDSExp" scope="application">	
		...
		<table style="width:100%;"><tr valign="top"><td>					
		<x:forEach select="$contentStandardsXmlDom_DDSExp/DDSWebService/ListContentStandards/contentStandards/contentStandard" varStatus="status">			
			<x:if select="contains( renderingGuidelines/noDisplay, 'false' )">	
	...
	<c:set var="collectionsCheckBoxMenu_DDSExp" scope="application">	
		<div>
		...
		<div style="margin-left:${indentAmount}px">
		<table style="width:100%;"><tr><td>
		<x:forEach select="$collectionsXmlDom_DDSExp/DDSWebService/ListCollections/collections/collection">
			<%-- Render the list items --%>
			<c:set var="searchKey"><x:out select="searchKey"/></c:set>		
			<c:if test="${not empty searchKey}">	
	...
	<c:set var="menusLoaded_DDSExp" value="true" scope="application"/>
</c:if>
