/* 
	This script implements a simple search page for a DDS or NCS repository.
...
function doServiceRequest(action) {
	...
	if(action == 'Search') {
		...
		if(parms.sl && typeof(smartLinkQueries) != 'undefined') {
			...
			if(boosterTerms) {
				if(typeof(boosterTerms) == 'string') boosterTerms = [boosterTerms]; 
				boosterTerms.each(function(term) {
					smartLinkQuery += ' AND ' + makeBooserQueryClause(term);	
				});
			}
		}
		...
		request = baseUrl + '?verb=Search&client=js_client&output=json&transform=localize&callback=serviceJsonCallback&s='+s+'&n='+numResults+'&q='+encodeURIComponent(query);	
	}
...
// This function boosts ranking of results that match the given term without changing the result set itself - AND this with other clauses:
function makeBooserQueryClause(term) {
	return '(allrecords:true OR title:('+term+')^3 OR titlestems:('+term+')^3 OR description:('+term+')^3)';
}
	...
	try {
		if(json != null && json.DDSWebService) {
			...
			// Handle display of Search results, if one or more records:
				...
				if(offset > 0)
					pager += '<a href="?action=Search'+qArgs+'&s='+(offset-numResults)+'">&#171; Prev</a> - ';
				if(parms.q)
					pager += 'Results ';
				pager += (offset+1) + ' - ' + (offset+numReturned) + ' out of ' + formatNumber(totalNumResults);				
				if( offset+numResults < totalNumResults)
					pager += ' - <a href="?action=Search'+qArgs+'&s='+(offset+numResults)+'">Next &#187;</a>';

				var resultsDisp = '';
				if(parms.q)
					resultsDisp += 'Results ';
				resultsDisp += (offset+1) + ' - ' + (offset+numReturned) + ' out of ' + formatNumber(totalNumResults);
				
				searchHTML += smartLinkLabelDisp + '<div class="resultsDisp">' + resultsDisp + '</div>';
				...
				searchHTML += '<div class="pager">' + pager + '</div>';
...
function getRecordDisplayHtml(recordJson,i) {
	var html = '';
	try {
		var title = getRecordTitle(recordJson);
		var url = getRecordUrl(recordJson);
		var description = getRecordDescription(recordJson)
		...
		if(!html.empty())
			html += '<div class="recordSource">From: ' + recordJson.head.collection.content + '</div>';			
		if(!url.empty())
...
function isStopWord(term) {
	return ENGLISH_STOP_WORDS.any(function(n) { return term.toLowerCase() == n } );
}
...
function getRecordTitle(recordJson) {
	try {
		...
		if(recordJson.metadata.itemRecord)
			value = recordJson.metadata.itemRecord.general.title;
		...
		// DLESE annotation record:
			...
			value = recordJson.metadata.annotationRecord.annotation.title;
		// DLESE news opps record:
		else if(recordJson.metadata['news-oppsRecord'])
			value = recordJson.metadata['news-oppsRecord'].title;	
		// Concept record:
		else if(recordJson.metadata.concept)
			value = recordJson.metadata.concept.shortTitle;
		// OAI DC record:
...
function getRecordDescription(recordJson){
	try {
		...
		// ADN record:
		if(recordJson.metadata.itemRecord)
			value = recordJson.metadata.itemRecord.general.description;
		// NCAR library_dc record:
		else if(recordJson.head.xmlFormat == 'library_dc')
		...
		// DLESE Collection record:
			...
			value = recordJson.metadata.collectionRecord.general.description;
		// DLESE annotation record:
		else if(recordJson.metadata.annotationRecord)
			value = recordJson.metadata.annotationRecord.annotation.content.description;
		// DLESE news opps record:
		else if(recordJson.metadata['news-oppsRecord'])
			value = recordJson.metadata['news-oppsRecord'].description;		
		// Concept record:
		else if(recordJson.metadata.concept)
			value = recordJson.metadata.concept.longTitle;
		// OAI DC record:
		else if(recordJson.metadata.dc)
...
function getRecordUrl(recordJson){
	try {
		...
		// NCAR library_dc record:
			...
			value = getContent(recordJson.metadata.record.URL,1,'http');			
		// ncs_item, MSP2 record:
		else if(recordJson.metadata.record)
			value = recordJson.metadata.record.general.url;		
		// DLESE Collection record:
		else if(recordJson.metadata.collectionRecord)
		...
		// OAI DC record:
			...
			value = getContent(recordJson.metadata.dc.identifier,1,'http');			
		// NSDL DC record:
		else if(recordJson.metadata.nsdl_dc)
			value = getContent(recordJson.metadata.nsdl_dc.identifier,1,'http');
		// NSDL NCS Collection record:
		else if(recordJson.head.xmlFormat == 'ncs_collect')
			value = getContent(recordJson.metadata.record.general.url,1,'http');		
		return (Object.isString(value) ? value : '');
	} catch (e) {
		return '';
	}
...
function getContent(path,maxElms,mustStartWith) {
	if(!path)
		return '';
	if(Object.isString(path))
		return path;
	if(path.content && Object.isString(path.content))
		return path.content;
	...
	if(Object.isArray(path)) {
		for(var i in path) {
			...
			if(str != null && str.length > 0 && (!mustStartWith || str.startsWith(mustStartWith))) {
				if(maxElms && maxElms == 1)
					value = str;
				else
					value += '<div style="margin-bottom: 4px;">'+str+'</div>';
