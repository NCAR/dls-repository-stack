...
<div style="position:absolute; top:4px; right: 4px" >
	<img src='<c:out value="${pageContext.request.contextPath}"/>/images/dds_logo_sm.gif' border="0"/>
</div>

<h2 style="padding: 0px; margin: 0px; position:absolute; top:8px; left: 6px" >DDS Help</h2>


<div style="padding:4px; padding-top:40px">
...
<i>http://some/provider.org/base/url?verb=ListSets</i><p>To form this request, concatenate together the [base URL] + [?verb=ListSets]
		...
		<c:if test="${param.help == 'repinfo_baseurl'}">
			...
			the web browser when connecting to the software. For example, if a user accesses DDS using 
			...
			to the same instance of DDS using the Internet address http://myserver.somewhere.edu${pageContext.request.contextPath}, the 
		...
		<c:if test="${param.help == 'set_lucene_query'}">
			...
			<p>
				&#187; <a href="${pageContext.request.contextPath}/services/ddsws1-1/service_specification.jsp#availableSearchFields" target="_blank">Read about the available search fields and search syntax</a>.
			</p>
		</c:if>			
	</c:if>
