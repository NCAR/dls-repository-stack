...
<div class="leaf-wrapper">
 ...
 <c:choose>
	...
	<c:when test='${inputMode == "multiple" && not fn:containsIgnoreCase(vocabTerm.label, "\'")}'>
		<c:set var="checkBoxId" value="${sf:pathToId(elementPath)}_${vocabTerm.name}_checkbox" />
		
 		<input type="checkbox" id="${checkBoxId}"
			name="${elementPath}"
			value="<c:out value="${vocabTerm.name}" escapeXml="false"/>"
			<c:if test="${sf:arrayContains (selectedValues, vocabTerm.name)}">checked</c:if>
		/><label class="${typeclass}"
			<c:if test="${not empty vocabTerm.definition}">title="<c:out value='${vocabTerm.definition}'/>"</c:if>
			for="${checkBoxId}"><c:out value="${vocabTerm.label}"/>
			</label>
			
	</c:when>
	<c:otherwise>
		<%-- <div style="color:red">ERROR: unknown inputMode: (${inputMode})</div> --%>
		<div style="color:red;padding-left:20px"><c:out value="${vocabTerm.label}" escapeXml="false" /></div>
	</c:otherwise>
</c:choose> 
</div>
