...
<!--A. PURPOSE-->
<!-- **************************************-->
<!--To transform Digital Library for Earth System Education (DLESE) annotation metadata records to simple Dublin Core that uses the Open Archives Initiative (OAI) namespace-->

<!--B. LICENSE INFORMATION and CREDITS-->
<!-- *****************************************************-->
<!--Date created: 2006-03-31 by Katy Ginger, DLESE Program Center, University Corporation for Atmospheric Research (UCAR)-->
...
<!--dc: publisher, dc:creator and dc:contributor from contributor-->
			<xsl:apply-templates select="a:annotation/a:contributors/a:contributor"/>
...
<!--Case 1: content URL exists and context URL does not exist-->
					<xsl:when test="string-length(a:annotation/a:content/a:url) > 0 and string-length(a:annotation/a:context) =0">
						...
						<xsl:apply-templates select="a:itemID">
...
<!-- Contact does not contribute to the content of the resource, so not using -->
	<xsl:template match="a:contributor">
		<xsl:if test="./@share='true'">
			<xsl:choose>
				<xsl:when test="./@role='Author'">
					<xsl:element name="dc:creator">
						<xsl:apply-templates select="a:person"/>
						<xsl:apply-templates select="a:organization"/>
					</xsl:element>
				</xsl:when>
...
<!--3. ORGANIZATION template-->
	<xsl:template match="a:organization">
		<xsl:choose>
