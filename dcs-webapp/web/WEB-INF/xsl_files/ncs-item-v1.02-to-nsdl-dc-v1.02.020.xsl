...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:dc="http://purl.org/dc/elements/1.1/" xmlns:nsdl_dc="http://ns.nsdl.org/nsdl_dc_v1.02/" xmlns:ncs="http://ns.nsdl.org/ncs" xmlns:dct="http://purl.org/dc/terms/" xmlns:ieee="http://www.ieee.org/xsd/LOMv1p0" xmlns:xs="http://www.w3.org/2001/XMLSchema" exclude-result-prefixes="xsi ncs" version="1.0">

<!--PURPOSE-->
<!-- **************************************-->
<!--To transform ncs_item version 1.02 metadata records to the nsdl_dc version 1.02.020 format.-->
...
<!-- *****************************************************-->
<!--Date created: 2007-08-10 by Katy Ginger, University Corporation for Atmospheric Research (UCAR)-->
...
<!--dc:relation when there is a partner URL (landing page) to a resource; doesn't really need a template but easy to do-->
		<xsl:apply-templates select="ncs:general/ncs:partnerURL" mode="process"> 
...
<!--NOTE: anything else does not get transformed-->
					<xsl:choose>
						<xsl:when test="(string-length($lowercase) = '4' and not(string(number($yearStr)) = 'NaN')) or         (string-length($lowercase) = '7' and not(string(number($yearStr)) = 'NaN') and ($firstSep = '-') and            not (string(number($monthStr)) = 'NaN') and ($monthStr &gt; '0' ) and ($monthStr &lt; '13' )) or        (string-length($lowercase) = '10' and not (string(number($yearStr)) = 'NaN')  and             ($firstSep = '-') and not (string(number($monthStr)) = 'NaN') and             ($monthStr &gt; '0' ) and ($monthStr &lt; '13' ) and ($secondSep = '-') and             not (string(number($dayStr)) = 'NaN') and ($dayStr &gt; '0' ) and ($dayStr &lt; '32' ))">
