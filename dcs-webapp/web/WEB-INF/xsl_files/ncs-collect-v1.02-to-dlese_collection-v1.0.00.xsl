...
<!--HISTORY-->
<!-- **************************************-->
<!--2010-02-25: No need to call the DDS web service because the key information that was being pulled is not what is wanted; when writing the dlese_collect key use the ncs_collect recordID value; when writing the dlese_collect recordID number (under metaMetadata) use ncs_collect recordID but preface it withDC-->
<!--assign ncs_collect record NAB-000-000-000-001 a dlese_collect key of 'collect' to work in DDS-->
...
<!--when there is an OAI format; assume it is written nicely as nsdl_dc or oai_dc-->
					<xsl:when test="string-length(ncs:collection/ncs:ingest/ncs:oai/@libraryFormat) > 0">
						<xsl:value-of select="ncs:collection/ncs:ingest/ncs:oai/@libraryFormat"/>
					</xsl:when>
<!--for the special record that is the dlese_collect collection of collection records; this when clause must come before the next when clause otherwise this collection will be assigned a format of nsdl_dc rather than dlese_collect-->
					<xsl:when test="ncs:general/ncs:recordID = 'NAB-000-000-000-001' ">
						<xsl:text>dlese_collect</xsl:text>
					</xsl:when>
<!--for the special record that is the NSDL collection of collection records; this when clause must come before the next when clause otherwise this collection will be assigned a format of nsdl_dc rather than ncs_collect-->
					<xsl:when test="ncs:general/ncs:recordID = 'NAB-000-000-000-006' ">
						<xsl:text>ncs_collect</xsl:text>
					</xsl:when>
<!--when there is no OAI harvest format assume nsdl_dc because it would come from the NCS as nsdl_dc-->
					<xsl:when test="string-length(ncs:collection/ncs:ingest/ncs:oai/@libraryFormat) = 0 ">
...
the above correctly grabs the field of key but this is not the value that is wanted. do the following below instead-->

<!--for figuring the key-->
<!--for the dlese_collect key use the ncs_collect recordID value unless it is record NAB-000-00-000-001 which needs a key of 'collect'-->
				<xsl:choose>
					<xsl:when test="ncs:general/ncs:recordID = 'NAB-000-000-000-001' ">
			...
			<metaMetadata>
				<catalogEntries>
<!--enter the NSDL collection record id as the DLESE collection record id but need to add a preface label like DC to make the NAB DDS not think there are 2 records with the same id #-->
					<xsl:element name="catalog" namespace="http://collection.dlese.org">
						<xsl:attribute name="entry">
							<xsl:value-of select="concat('DC-', ncs:general/ncs:recordID)"/>
						</xsl:attribute>
