<%-- $Id: view_collection_config.jsp,v 1.4 2008/10/14 17:41:03 ostwald Exp $ --%>
<%@ include file="/JSTLTagLibIncludes.jsp" %>
<jsp:useBean id="sessionBean" class="edu.ucar.dls.schemedit.SessionBean" scope="session"/>
<c:set var="contextPath"><%@ include file="/ContextPath.jsp" %></c:set>
<c:set var="title">${viewForm.dcsSetInfo.name}</c:set>
...
<st:breadcrumbs>
	...
	<st:breadcrumbArrow />
	<a  href="../admin/admin.do?page=collections">Collection Settings</a>
	<st:breadcrumbArrow />
...
<table cellpadding="5" cellspacing="0" border="0">
	<tr align="left">
		<c:choose>
			<c:when test="${viewForm.collectionFrameworkLoaded}">
				<td>
					<c:set var="editorHref" 
						  value="../editor/collection_config.do?collection=${viewForm.dcsSetInfo.setSpec}&recId=${viewForm.dcsSetInfo.setSpec}&command=edit&src=local"/>
			...
			<c:otherwise>
				<td>
					<input type="button" value="Edit Settings" class="record-action-button-disabled"/>
				</td>
				<td>
					Settings cannot be edited because the collection's framework is not loaded.
				</td>
			</c:otherwise>
		</c:choose>
	</tr>
...
<%@ include file="/browse/viewing/collection_config_record.jsp" %>
