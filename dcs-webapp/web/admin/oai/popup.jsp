...
<i>http://some/provider.org/base/url?verb=ListSets</i><p>To form this request, concatenate together the [base URL] + [?verb=ListSets]
		...
		<c:if test="${param.help == 'dm_provider'}">
			<b>Provider Access</b> - A toggle that enables or disables the metadata directory so that metadata files may or may not be 
		...
		<c:if test="${param.help == 'set_lucene_query'}">
			...
			<p>
				<a href="http://www.dlese.org/dds/services/ddsws1-0/service_specification.html#availableSearchFields" target="_blank">Read about the Lucene search syntax</a> and the search fields that are available for common DLESE metadata formats.
			</p>
		</c:if>			
	</c:if>
