...
<c:catch var="xmlParseError">
	...
	<x:set var="rootElement" select="$myRec/proj_profile"/>							
</c:catch>
<c:choose>
	...
	<c:otherwise>
		...
		<c:set var="title">
			<x:out select="$rootElement/profileName"/>
		</c:set>						
		<c:set var="annotates">
		...
		<c:if test="${not empty annotates}">
			<div class="searchResultValues"><em>Annotates:</em> &nbsp;
				<dds:keywordsHighlight>${annotates}</dds:keywordsHighlight>
			</div>
		</c:if>
