<c:if test="${(param['response.mode'] == 'allOff' && f:contains(paramValues['response'],'head')) || param['response.mode'] != 'allOff'}">
	<head><%-- Outputs the head element. Place the record's doc reader in var 'headDocReader' before including --%>
		...
		<c:choose>
			<c:when test="${not empty df11.recordFormat}"> 
				<c:set var="xmlFormat" value="${df11.recordFormat}"/>
			</c:when>
			<c:otherwise>
				<c:set var="xmlFormat" value="${headDocReader.nativeFormat}"/>
			</c:otherwise>
		</c:choose>
		<xmlFormat>${xmlFormat}</xmlFormat> <c:set value="${headDocReader.collection}" target="${df11}" property="value" /><c:set target="${df11}" value="dlese_collect" property="metaFormat" /> <c:set value="key" target="${df11}" property="field" />			
		...
		<c:choose>
			...
			<c:when test="${headDocReader.nativeFormat == 'osm_collect' ||
											headDocReader.nativeFormat == 'ncs_collect'}" >
				<%@ include file="CollectionRecordAdditionalMetadata.jsp" %>
			</c:when>
